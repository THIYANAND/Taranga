...
class PPMWindow(QWidget):

    def __init__(self):

        super().__init__()

        self.setWindowTitle("PPM Modulation")
    ...
    def setup_ui(self):
        ...
        # TITLE
        title = QLabel("PULSE POSITION MODULATION")
        ...
        # DEFINITION
        definition = QLabel(
            "Pulse Position Modulation changes pulse position according to message signal amplitude."
        )
        ...
        # FORMULA
        formula = QLabel(
            "Pulse Position ∝ Message Signal Amplitude"
        )
        ...
        self.pulse_plot.setTitle(
            "Reference Pulse Train",
            color="yellow",
            size="14pt"
        )

        self.ppm_plot.setTitle(
            "PPM Signal",
            color="lightgreen",
            size="14pt"
        )
        ...
        self.ppm_curve = self.ppm_plot.plot(
            pen=pg.mkPen(
                '#00ffff',
                width=2
            )
        ...
        self.pulse_plot.scene().sigMouseMoved.connect(
            self.pulse_mouse_moved
        )

        self.ppm_plot.scene().sigMouseMoved.connect(
            self.ppm_mouse_moved
        )
    ...
    def reset_view(self):
        ...
        self.pulse_plot.setXRange(
            -0.2,
            1.2
        )

        self.pulse_plot.setYRange(
            -0.5,
            1.5
        )

        self.ppm_plot.setXRange(
            -0.2,
            1.2
        )

        self.ppm_plot.setYRange(
            -0.5,
            1.5
        )
    ...
    def pulse_mouse_moved(self, pos):

        vb = self.pulse_plot.plotItem.vb

        point = vb.mapSceneToView(pos)

        self.cursor_label.setText(

            f"[PULSE TRAIN]   Time: {point.x():.4f} s    Amplitude: {point.y():.4f}"

        )

    def ppm_mouse_moved(self, pos):

        vb = self.ppm_plot.plotItem.vb

        point = vb.mapSceneToView(pos)

        self.cursor_label.setText(

            f"[PPM SIGNAL]   Time: {point.x():.4f} s    Amplitude: {point.y():.4f}"
