...
class QPSKWindow(QWidget):

    def __init__(self):

        super().__init__()

        self.setWindowTitle("QPSK Modulation")
    ...
    def setup_ui(self):
        ...
        # TITLE
        title = QLabel("QUADRATURE PHASE SHIFT KEYING")
        ...
        # DEFINITION
        definition = QLabel(
            "Quadrature Phase Shift Keying uses four different carrier phases to transmit data."
        )
        ...
        self.qpsk_curve = self.qpsk_plot.plot(
            pen=pg.mkPen(
                '#00ffff',
                width=2
            )
        ...
        self.qpsk_plot.scene().sigMouseMoved.connect(
            self.qpsk_mouse_moved
        )
    ...
    def reset_view(self):
        ...
        self.qpsk_plot.setXRange(
            -0.2,
            1.2
        )

        self.qpsk_plot.setYRange(
            -amplitude - 1,
            amplitude + 1
        )
    ...
    def qpsk_mouse_moved(self, pos):

        vb = self.qpsk_plot.plotItem.vb

        point = vb.mapSceneToView(pos)

        self.cursor_label.setText(

            f"[QPSK SIGNAL]   Time: {point.x():.4f} s    Amplitude: {point.y():.4f}"
    ...
    def update_waveforms(self):
        ...
        # PHASE
        phase_signal = np.arctan2(
            q_signal,
            i_signal
        )

        # QPSK SIGNAL
        qpsk_signal = amplitude * np.cos(
