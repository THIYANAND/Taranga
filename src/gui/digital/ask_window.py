...
class ASKWindow(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("ASK Modulation")
    ...
    def setup_ui(self):
        ...
        # TITLE
        title = QLabel("AMPLITUDE SHIFT KEYING")
        ...
        # DEFINITION
        definition = QLabel(
            "Amplitude Shift Keying varies carrier amplitude according to digital binary data."
        )
        ...
        self.ask_curve = self.ask_plot.plot(
            pen=pg.mkPen(
                '#00ffff',
                width=2
            )
        ...
        self.ask_plot.scene().sigMouseMoved.connect(
            self.ask_mouse_moved
        )
    ...
    def reset_view(self):
        ...
        self.ask_plot.setXRange(
            -0.2,
            1.2
        )

        self.ask_plot.setYRange(
            -amplitude - 1,
            amplitude + 1
        )
    ...
    def ask_mouse_moved(self, pos):

        vb = self.ask_plot.plotItem.vb

        point = vb.mapSceneToView(pos)

        self.cursor_label.setText(

            f"[ASK SIGNAL]   Time: {point.x():.4f} s    Amplitude: {point.y():.4f}"
