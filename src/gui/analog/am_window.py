...
class AMWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        # TITLE
        title = QLabel("AMPLITUDE MODULATION")
        ...
        # DEFINITION
        definition = QLabel(
            "Amplitude Modulation varies carrier amplitude according to message signal."
        )
        ...
        self.mod_index_input = QLineEdit("0.5")
        ...
        self.am_curve = self.am_plot.plot(
            pen=pg.mkPen(
                '#00ffff',
                width=2
            )
        ...
        self.am_plot.scene().sigMouseMoved.connect(
            self.am_mouse_moved
        )
    ...
    def reset_view(self):
        ...
        mu = self.mod_index_slider.value() / 100

        max_am = (1 + mu * amplitude) + 1
        ...
        self.am_plot.setXRange(
            -0.2,
            1.2
        )

        self.am_plot.setYRange(
            -max_am,
            max_am
        )
    ...
    def update_inputs(self):
        ...
        self.mod_index_input.setText(
            str(
                self.mod_index_slider.value() / 100
            )
        )
    ...
    def am_mouse_moved(self, pos):

        vb = self.am_plot.plotItem.vb

        point = vb.mapSceneToView(pos)

        self.cursor_label.setText(

            f"[AM SIGNAL]   Time: {point.x():.4f} s    Amplitude: {point.y():.4f}"
